...
public class Wizard implements MagicSource, Trader, MagicEffectRealization {
	...
	public boolean castRandomSpell(MagicEffectRealization target) {
        ...
        int randomIndex = (int) (Math.random() * knownSpells.size());
	...
	public boolean useRandomItem(MagicEffectRealization target) {
	    ...
	    Tradeable randomItem = iterator.next();
	...
	public boolean sellItem(Tradeable item, Trader target) {
		 if (item == null || target == null) {
		        throw new IllegalArgumentException("Item and target cannot be null.");
		    }
	...
	public boolean sellRandomItem(Trader target) {
	    ...
	    // Get the random item by iterating to the random index
	    for (int i = 0; i < randomIndex; i++) {
	        iterator.next();
	    }
	...
	public String toString() {
		String levelString = level.toString(); // Assuming level.toString() returns the asterisks representation
	    ...
	    return String.format("[%s(%s): %d/%d %d/%d; %d %s; knows %s; carries %s]",
	            name, levelString, HP, basicHP, MP, basicMP, money, knutOrKnuts, knownSpellsString, inventoryString);
	}
	...
	public boolean steal(Trader thief) {
		if (thief == null) {
	        throw new IllegalArgumentException("Invalid thief");
	    }
	    if (!canSteal()) {
	        return false;
	    }
	    ...
	    if (randomItem != null && thief.hasCapacity(randomItem.getWeight())) {
	...
	public boolean loot(Trader looter) {
	    ...
	    Set<Tradeable> itemsToTransfer = new HashSet<>(inventory);
	    ...
	    return !itemsToTransfer.isEmpty();
	}
	...
	public void weakenMagicPercent(int percentage) {
		double magicReduction = basicMP * (percentage / 100.0);
	    int reductionAmount = (int) magicReduction;
	    MP -= reductionAmount;
	    if (MP < 0) 
	    {
	        MP = 0;
	...
	public void enforceMagicPercent(int percentage) {
		double magicIncrease = basicMP * (percentage / 100.0);
	    int increaseAmount = (int) magicIncrease;
	    MP += increaseAmount;
	}

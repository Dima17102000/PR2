	...
	*/
	...
	public void doEffect(MagicEffectRealization target) {
		...
		if (target.isProtected(this)) {
			Set <AttackingSpell> protectionSpells = new HashSet<>();
            protectionSpells.add(this);
            target.removeProtection(protectionSpells);
        } else {
            if (type) {
                if (percentage) {
	...
	public String additionalOutputString() {
        ...
        String HPorMP = (type) ? "MP" : "HP";
        return "; -" + amount + sign + " " + HPorMP;
	}

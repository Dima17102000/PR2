...
public class Concoction extends Potion {
	...
	public String additionalOutputString() {
		...
		StringBuilder output = new StringBuilder(super.additionalOutputString());

	    // Add health if it is non-zero
	    if (health != 0) {
	        output.append("; ").append(health > 0 ? "+" : "-").append(Math.abs(health)).append(" HP");
	    }

	    // Add mana if it is non-zero
	    if (mana != 0) {
	        output.append("; ").append(mana > 0 ? "+" : "-").append(Math.abs(mana)).append(" MP");
	    }

	    // Add spells if the list is not empty
	    /*if (!spells.isEmpty()) {
